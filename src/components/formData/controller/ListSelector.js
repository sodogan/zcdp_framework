sap.ui.define([
	...
	"use strict";
	return BaseObject.extend("aklc.cm.components.formData.controller.ListSelector", {
		bMainResolve: false,
		/**
		 * Provides a convenience API for selecting list items. All the functions will wait until the initial load of the a List passed to the
		 * instance by the setBoundMasterList function.
		 * 
		...
		constructor: function(oComponent) {
			...
			this.oWhenListLoadingIsDone = new Promise(function(fnResolve, fnReject) {
				...
				this._oWhenListHasBeenSet.then(function(oList) {
					oList.getBinding("items").attachEventOnce("dataReceived", function(oEvent) {
		...
		/**
		 * A bound list should be passed in here. Should be done, before the list has received its initial data from the server. May only be invoked
		 * once per ListSelector instance.
		 * 
		...
		/**
		 * Tries to select a list item with a matching binding context. If there are no items matching the binding context or the ListMode is none, no
		 * selection will happen
		 * 
		 * @param {string} sBindingPath the binding path matching the binding path of a list item
		 */
		selectAListItem: function(sBindingPath) {
			jQuery.sap.log.info("MockRequest:FormData:ListSelector:selectAListItem");
			...
			this.oWhenListLoadingIsDone.then(function() {
				...
				oList.getItems().some(function(oItem) {
					if (oItem.getBindingContext() && oItem.getBindingContext().getPath() === sBindingPath) {
						...
						oList._fireSelectionChangeEvent([
							oItem
						]);
						oItem.focus();
						return true;
		...
		/**
		 * Attaches a listener and listener function to the ListSelector's bound master list. By using a promise, the listener is added, even if the
		 * list is not available when 'attachListSelectionChange' is called.
		 * 
		...
		/**
		 * Detaches a listener and listener function from the ListSelector's bound master list. By using a promise, the listener is removed, even if
		 * the list is not available when 'detachListSelectionChange' is called.
		 * 
		 * @param {function} fnFunction the function to be executed when the list fires a selection change event
		 * @param {function} oListener the listener object
		 * @return {sap.ui.demo.masterdetail.model.ListSelector} the list selector object for method chaining
		...
		/**
		 * Removes all selections from master list. Does not trigger 'selectionChange' event on master list, though.
		 * 
		 * @public
		 */
		...
		selectNextStep: function() {
			...
			jQuery.sap.log.info("MockRequest:FormData:ListSelector:selectNextStep");

			if ((iCurrentIndex + 1 === aItems.length) || (this.bMainResolve === true)) {
